 ...
 *
 */
public class SAPConnectorUpdateOperation extends SizeLimitedUpdateOperation{

	
	private static final Logger logger = Logger.getLogger(SAPConnectorUpdateOperation.class.getName());
	...
	protected SAPConnectorUpdateOperation(SAPConnectorConnection conn) {
		super(conn);
	}
	...
	protected void executeSizeLimitedUpdate(UpdateRequest request, OperationResponse response) {
		...
		for(ObjectData objData : lTrackedData) {
				...
				try {
					...
					if(rootNode != null) {
						updateRequest.setEntity(new StringEntity(rootNode.toString(), ContentType.APPLICATION_JSON));
					}else {
						updateRequest.setEntity(new InputStreamEntity(objData.getData(),ContentType.APPLICATION_JSON));
					}
					updateRequest.setHeader(CONTENT_HEADER);
					updateRequest.setHeader(ACCEPT_HEADER);
					updateRequest.setHeader("Accept", "application/json");
					updateRequest.setHeader("Accept-Language","application/json");
					...
					logger.log(Level.INFO,"Update Respones StatusCode : {0}",statusCode);
					
					if(statusCode == HttpStatus.SC_NO_CONTENT) {
						ResponseUtil.addSuccess(response, objData, StringUtil.toString(statusCode));
					}else {
					...
					logger.log(Level.SEVERE,"Exception while Update Operation : {0}",e.getMessage());
	 ...
	 */
	private List<ObjectData> getTrackedData(UpdateRequest request){
